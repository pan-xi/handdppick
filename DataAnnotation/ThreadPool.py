...
def get_answer_from_llm(llm_model, messages, api_collections, dataset_name=None, file_path=None, retry_count=3, retry_delay=1):
    ...
    for attempt in range(retry_count):
        try:
            ...
            try:
                raw_response = response.text  # 先获取原始文本
                print("Raw Response:", raw_response)  # 调试用
                response_json = response.json()
            except json.JSONDecodeError as e:
                logging.error(f"Raw Content: {raw_response}")  # 记录原始内容
                try:
...
def analyze_code_with_llm(llm_model, code, api_collections, dataset_name, file_path):
    ...
    count += 1
    function_description = get_answer_from_llm(llm_model, prompt, api_collections, dataset_name, file_path)
    ...
    prompt = [{"role": "system", "content": system_prompt_list[count]}] + history
    count += 1
    vulnerability_analysis = get_answer_from_llm(llm_model, prompt, api_collections, dataset_name, file_path)
    if not vulnerability_analysis or "{'CWE_Type':'pass!'}" in vulnerability_analysis:
        return function_description, vulnerability_analysis, None
    ...
    prompt = [{"role": "system", "content": system_prompt_list[count]}] + history
    repaired_code = get_answer_from_llm(llm_model, prompt, api_collections, dataset_name, file_path)
...
def process_single_record(args):
    ...
    function_description, vulnerability_analysis, repaired_code = analyze_code_with_llm(
...
def main():
    """主函数"""
    input_file = './annotation_results/split1_rest.json'
    llm_model = 'deepseek-chat'
    # 获取input_file的文件名
    input_file_name = os.path.basename(input_file).replace('.json','')
    # 拼上模型名字
    output_file = f'./annotation_results/{input_file_name}_output_deepseekv3_official.json'
    split_size = 20
